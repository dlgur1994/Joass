...
input_wav = '/var/www/html/input.wav'

data = extract_feature(input_wav)

model = ImportGraph(main_model_path)
result, ans_list = model.run(data, 9)

if result == '소리작게' or result == '소리크게':
    model_ = ImportGraph(sound_model_path)
    result, ans_list = model_.run(data, 2)

if result == '오늘날씨' or result == '오늘일정':
    model_ = ImportGraph(day_model_path)
    result, ans_list = model_.run(data, 3)
...
for ans in range(9):
    ...
    print(output + ": ", ans_list[ans])

outfile = open('/var/www/html/new.php', 'w')
